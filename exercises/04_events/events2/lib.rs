// events2
//
// This exercise teaches you about event topics and using multiple events.
//
// Topics make events searchable and filterable. You can have up to 4 topics
// per event (including the event signature itself). Use topics on fields
// you want to query or filter by.
//
// Your task: Complete a simple token tracker that emits events for transfers
//
// Execute `inklings verify 04_events/events2` when you think you're done!
...
mod events2 {
    ...
    #[ink(event)]
    pub struct Transfer {
        // TODO: Add #[ink(topic)] to from so we can query "all transfers FROM this account"
       
        from: Option<H160>,
        // TODO: Add #[ink(topic)] to to so we can query "all transfers TO this account"
      
        to: Option<H160>,
        // The amount is NOT a topic - we don't typically filter by amount
        value: Balance,
    }

    /// Event emitted when someone approves another account to spend tokens
    // TODO: Add the #[ink(event)] attribute
    todo!()
    pub struct Approval {
        // TODO: Add topics to owner and spender
    ...
    impl Events2 {
        ...
        pub fn transfer(&mut self, to: H160, value: Balance) -> Result<()> {
            ...
            // Hint: from should be Some(from), to should be Some(to)
        ...
        pub fn mint(&mut self, to: H160, value: Balance) -> Result<()> {
            ...
            // TODO: Emit a Transfer event with from = None (minting event)
            // This is a common pattern to indicate tokens are being created
