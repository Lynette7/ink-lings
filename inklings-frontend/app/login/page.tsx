...
export default function LoginPage() {
  ...
  const handleWalletConnect = async () => {
    setError("")
    // Mock wallet connection - in real app, use Web3.js or ethers.js
    const mockWalletAddress = "0x" + Math.random().toString(16).slice(2, 42)
    try {
      await login(mockWalletAddress, "wallet_auth")
      router.push("/exercises")
    } catch (err) {
      ...
      <Card className="w-full max-w-md">
        ...
        <CardContent>
          ...
          <div className="mt-6 text-center text-sm">
            ...
            <Link href="/signup" className="text-[#6552D0] hover:underline">
              Sign up
            </Link>
          </div>
        </CardContent>
